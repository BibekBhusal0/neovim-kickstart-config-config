...
local default_opts = {
  unique_line_sign_text = require("utils.icons").others.ai,
...
local function start_spinners(bufnr, ns_id, start_line, end_line)
  local block_spinner = require("lua.plugins.ai.codecompanion.spinner.block_spinner").new {
    bufnr = bufnr,
    ns_id = ns_id,
  ...
  local spinner = require("lua.plugins.ai.codecompanion.spinner.spinner").new {
    bufnr = bufnr,
    ns_id = ns_id,
