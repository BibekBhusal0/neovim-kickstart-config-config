...
local function run_system_async(command, cwd, name, done)
  local ok, err = pcall(vim.system, command, { cwd = cwd }, function(result)
    if result and result.code and result.code ~= 0 then
      ...
      vim.notify(
        ("Build failed for `%s` (exit %d)%s"):format(name, result.code, suffix),
        vim.log.levels.WARN
      )
    end
    done()
...
local function run_build_step(ev, name, step, done)
  ...
  if type(step) == "string" then
    ...
    run_system_async(
      { vim.o.shell, vim.o.shellcmdflag, step },
      ev.data and ev.data.path or nil,
      name,
      done
    )
    return
    ...
    callback = function(ev)
      ...
      if not name then
        if not warned_missing_name then
          ...
          vim.notify(
            "Skipping build: PackChanged event is missing `spec.name`",
            vim.log.levels.WARN
          )
        end
        return
